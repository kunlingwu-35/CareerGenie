# [Career Genie Chatbot: Kode with Klossy Group D4 2026]

[Career Genie is an AI-powered web application designed to help identify what your dream career is, and create a roadmap to make your wish come true! ]

🤗 **Originally built as a Hugging Face Space:** [(https://huggingface.co/spaces/kode-with-klossy/4.3-groupD4-capstone)]

> ⚠️ Note: This Space is no longer live. The code in this repo is the full project.

![Screenshot of my chatbot](screenshot.png)

## What it does


- [Feature 1: A Personality Quiz at the start of the user journey to analyze their personality traits to help determine the best fit major.]
- [Feature 2: An AI Chatbot that:
Helps link best career options to majors
Helps find best colleges offering the major
Provides resources to guide user]
- [Feature 3: A quickNotes tab for user to record down careers of interest]

## How it works

[Generative AI for personalized conversations.
Uses user’s hobbies, goals, aspirations, and strengths and context.
System promoting guides relevant responses.
Generates career and education pathway driven suggestions.
Supports follow up questions for deeper exploration.
Creates a different experience based on each user’s individual needs.
We gathered structured career and education information to ground our project in reliable data.
We organized larger career information into smaller and relevant sections that are easier to work with (Information Chunking)]

## Built with

- **Gradio** — the chatbot interface
- **Hugging Face Inference Providers** — the AI model ([Qwen/Qwen2.5-7B-Instruct for generating personalized responses])
- **Sentence Transformers** — [converting user queries and career/major data into embeddings for semantic search]
- **Pandas** - [loading, cleaning, and organizing career and college datasets]

## What I learned

[The hardest part of building this for me was integrating databases into text format and breaking down into chunks. I had some trouble understanding the code, but working through them eventually helped with that. ]

## About

Built at [Kode With Klossy](https://www.kodewithklossy.com) AI/ML Camp,
Summer 2026, by [Kunling Wu, Niral J., Julia R., Soha K.].
